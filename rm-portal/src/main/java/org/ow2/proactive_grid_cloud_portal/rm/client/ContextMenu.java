    ...
    public static Menu createContextMenuFromCompactView(RMController controller, Object releated) {
        ...
        if (menu.nodesource != null) {
            switch (menu.nodesource.getNodeSourceStatus()) {
                case NODES_DEPLOYED:
                    menu.enableItems(new MenuItem[] { undeployItem });
                    menu.disableItems(new MenuItem[] { deployItem, editItem });
                    break;
                case NODES_UNDEPLOYED:
                    menu.enableItems(new MenuItem[] { deployItem, editItem });
                    menu.disableItems(new MenuItem[] { undeployItem });
                    break;
                default:
                    menu.disableItems(new MenuItem[] { deployItem, undeployItem, editItem });
            }
        } else {
            menu.disableItems(new MenuItem[] { deployItem, undeployItem, editItem });
        }
